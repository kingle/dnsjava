...
public final class Address {
  ...
  public static int[] toArray(String s, int family) {
    ...
    for (int i = 0; i < byteArray.length; i++) intArray[i] = byteArray[i] & 0xFF;
    return intArray;
  }
  ...
  public static InetAddress[] getAllByName(String name) throws UnknownHostException {
    try {
      ...
      for (int i = 0; i < records.length; i++) addrs[i] = addrFromRecord(name, records[i]);
      return addrs;
    }
  }
  ...
  public static InetAddress truncate(InetAddress address, int maskLength) {
    ...
    for (int i = maskLength / 8 + 1; i < bytes.length; i++) bytes[i] = 0;
    int maskBits = maskLength % 8;
    int bitmask = 0;
    for (int i = 0; i < maskBits; i++) bitmask |= (1 << (7 - i));
