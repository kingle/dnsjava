...
public class Header implements Cloneable {
  ...
  Header(DNSInput in) throws IOException {
    this(in.readU16());
    flags = in.readU16();
    for (int i = 0; i < counts.length; i++) counts[i] = in.readU16();
  }
  ...
  void toWire(DNSOutput out) {
    out.writeU16(getID());
    out.writeU16(flags);
    for (int i = 0; i < counts.length; i++) out.writeU16(counts[i]);
  }
  ...
  boolean[] getFlags() {
    ...
    for (int i = 0; i < array.length; i++) if (validFlag(i)) array[i] = getFlag(i);
    return array;
  }
  ...
  String toStringWithRcode(int newrcode) {
    ...
    sb.append("; ");
    for (int i = 0; i < 4; i++) sb.append(Section.string(i) + ": " + getCount(i) + " ");
    return sb.toString();
  }
