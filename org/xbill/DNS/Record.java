...
public abstract class Record implements Cloneable, Comparable, Serializable {
  ...
  /**
   * Returns the type of RRset that this record would belong to. For all types except RRSIG, this is
   * equivalent to getType().
   *
   * @return The type of record, if not RRSIG. If the type is RRSIG, the type covered is returned.
   * @see Type
   * @see RRset
   * @see SIGRecord
   */
  ...
  public int hashCode() {
    ...
    int code = 0;
    for (int i = 0; i < array.length; i++) code += ((code << 3) + (array[i] & 0xFF));
    return code;
  }
