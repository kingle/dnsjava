...
public class Name implements Comparable, Serializable {
  ...
  static {
    ...
    for (int i = 0; i < lowercase.length; i++) {
      if (i < 'A' || i > 'Z') lowercase[i] = (byte) i;
      else lowercase[i] = (byte) (i - 'A' + 'a');
    }
    root = new Name();
  ...
  private final int offset(int n) {
    ...
    if (n < MAXOFFSETS) {
      ...
      for (int i = MAXOFFSETS - 1; i < n; i++) pos += (name[pos] + 1);
      return (pos);
    }
  }
  ...
  private static final void copy(Name src, Name dst) {
    if (src.offset(0) == 0) {
      ...
      for (int i = 0; i < labels && i < MAXOFFSETS; i++) dst.setoffset(i, src.offset(i) - offset0);
  ...
  public Name(Name src, int n) {
    ...
    name = src.name;
    setlabels(slabels - n);
    for (int i = 0; i < MAXOFFSETS && i < slabels - n; i++) setoffset(i, src.offset(i + n));
  }
  ...
  public byte[] toWireCanonical() {
    ...
    for (int i = 0, spos = offset(0), dpos = 0; i < labels; i++) {
      ...
      if (len > MAXLABEL) throw new IllegalStateException("invalid label");
      b[dpos++] = name[spos++];
      for (int j = 0; j < len; j++) b[dpos++] = lowercase[(name[spos++] & 0xFF)];
    }
    return b;
  }
  ...
  private final boolean equals(byte[] b, int bpos) {
    ...
    for (int i = 0, pos = offset(0); i < labels; i++) {
      ...
      if (len > MAXLABEL) throw new IllegalStateException("invalid label");
      for (int j = 0; j < len; j++)
        if (lowercase[(name[pos++] & 0xFF)] != lowercase[(b[bpos++] & 0xFF)]) return false;
    }
    return true;
  }
